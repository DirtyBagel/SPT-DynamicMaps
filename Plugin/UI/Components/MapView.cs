...
public class MapView : MonoBehaviour
{
	...
	{
		var marker = MapMarker.Create(MapMarkerContainer, markerDef, _markerSize, -CoordinateRotation,
			1f / ZoomCurrent);
		AddMapMarker(marker);
		return marker;
	}

	public TransformMapMarker AddTransformMarker(Transform followingTransform, string name, string category,
		Color color,
	...
	public void SetMinMaxZoom(RectTransform parentTransform)
	{
		...
		ZoomMin = Mathf.Min(parentTransform.sizeDelta.x / mapSize.x, parentTransform.sizeDelta.y / mapSize.y) /
		          _zoomMinScaler;
