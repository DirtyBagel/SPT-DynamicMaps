...
public static class QuestUtils
{
	...
	private static PropertyInfo _questControllerQuestsProperty =
		AccessTools.Property(typeof(AbstractQuestControllerClass), "Quests");

	private static FieldInfo _questsListField =
		AccessTools.Field(_questControllerQuestsProperty.PropertyType, "list_1");

	private static MethodInfo _questsGetConditionalMethod =
		AccessTools.Method(_questControllerQuestsProperty.PropertyType, "GetConditional",
	...
	private static FieldInfo _conditionCounterTemplateField =
		AccessTools.Field(typeof(ConditionCounterCreator), "_templateConditions");

	private static FieldInfo _templateConditionsConditionsField =
		AccessTools.Field(_conditionCounterTemplateField.FieldType, "Conditions");

	private static FieldInfo _conditionListField =
		AccessTools.Field(_templateConditionsConditionsField.FieldType, "list_0");
	//
	...
	internal static IEnumerable<MapMarkerDef> GetMarkerDefsForPlayer(Player player)
	{
		...
		{
			DynamicMapsPlugin.Log.LogWarning(
				$"TriggersWithIds null: {TriggersWithIds == null} or QuestItems null: {QuestItems == null}");
			return null;
		}
		...
		string conditionDescription)
	{
		switch (condition)
		{
			...
			{
				foreach (var position in GetPositionsForQuestItems(findItemCondition.target, questName,
			...
			case ConditionCounterCreator conditionCreator:
			{
				...
				foreach (var position in GetPositionsForConditionCreator(conditionCreator, questName,
					         conditionDescription))
				{
					yield return position;
				}
	...
	private static IEnumerable<Condition> GetIncompleteQuestConditions(Player player, QuestDataClass quest)
	{
		...
		{
			DynamicMapsPlugin.Log.LogError(
				$"GetIncompleteQuestConditions: quest.Template.Conditions is null, skipping quest");
			yield break;
		}

		// TODO: conditions is a GClass reference
		if (!quest.Template.Conditions.TryGetValue(EQuestStatus.AvailableForFinish, out var conditions) ||
		    conditions == null)
		{
			DynamicMapsPlugin.Log.LogError(
				$"Quest {quest.Template.NameLocaleKey.BSGLocalized()} doesn't have conditions marked AvailableForFinish, skipping it");
			yield break;
		}
		...
		foreach (var condition in conditions)
		{
			...
			{
				DynamicMapsPlugin.Log.LogWarning(
					$"Quest {quest.Template.NameLocaleKey.BSGLocalized()} has null condition, skipping it");
				continue;
			}
	...
	private static IEnumerable<TriggerWithId> GetZoneTriggers(this IEnumerable<TriggerWithId> triggerWithIds,
		string zoneId)
	{
