...
public class TransformMapMarker : MapMarker
{
	...
	public static TransformMapMarker Create(Transform followingTransform, GameObject parent, string imagePath,
		Color color,
		...
		if (!FollowingTransform.gameObject.activeInHierarchy)
		{
			if (!_warnedAttachedIsDisabled)
			{
				DynamicMapsPlugin.Log.LogWarning(
					$"FollowingTransform for TransformMapMarker has been disabled without removing the map marker");
				Color = Color.red;
