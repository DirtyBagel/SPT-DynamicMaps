...
internal class ModdedMapScreen : MonoBehaviour
{
		...
		foreach (var dynamicProvider in _dynamicMarkerProviders.Values)
		{
			...
			catch (Exception e)
			{
				DynamicMapsPlugin.Log.LogError(
					$"Dynamic marker provider {dynamicProvider} threw exception in OnRaidEnd");
	...
	private void AdjustForInRaid(bool playAnimation)
	{
		...
		// adjust mask
		_scrollMask.GetRectTransform().DOSizeDelta(RectTransform.sizeDelta + MaskSizeModifierInRaid,
			_transitionAnimations ? speed : 0f);
	...
	private void OnShowInRaid(bool playAnimation)
	{
		...
		foreach (var dynamicProvider in _dynamicMarkerProviders.Values)
		{
			...
			{
				DynamicMapsPlugin.Log.LogError(
					$"Dynamic marker provider {dynamicProvider} threw exception in OnShowInRaid");
				DynamicMapsPlugin.Log.LogError($"  Exception given was: {e.Message}");
				DynamicMapsPlugin.Log.LogError($"  {e.StackTrace}");
			}
		}
	...
	private void OnHideInRaid()
	{
		foreach (var dynamicProvider in _dynamicMarkerProviders.Values)
		{
			...
			{
				DynamicMapsPlugin.Log.LogError(
					$"Dynamic marker provider {dynamicProvider} threw exception in OnHideInRaid");
				DynamicMapsPlugin.Log.LogError($"  Exception given was: {e.Message}");
				DynamicMapsPlugin.Log.LogError($"  {e.StackTrace}");
			}
		}
	...
	private void OnShowOutOfRaid()
	{
		...
		foreach (var dynamicProvider in _dynamicMarkerProviders.Values)
		{
			...
			{
				DynamicMapsPlugin.Log.LogError(
					$"Dynamic marker provider {dynamicProvider} threw exception in OnShowOutOfRaid");
				DynamicMapsPlugin.Log.LogError($"  Exception given was: {e.Message}");
				DynamicMapsPlugin.Log.LogError($"  {e.StackTrace}");
			}
		}
	...
	private void OnHideOutOfRaid()
	{
		foreach (var dynamicProvider in _dynamicMarkerProviders.Values)
		{
			...
			{
				DynamicMapsPlugin.Log.LogError(
					$"Dynamic marker provider {dynamicProvider} threw exception in OnHideOutOfRaid");
				DynamicMapsPlugin.Log.LogError($"  Exception given was: {e.Message}");
				DynamicMapsPlugin.Log.LogError($"  {e.StackTrace}");
			}
		}
	...
	private void ChangeMap(MapDef mapDef)
	{
		...
		foreach (var dynamicProvider in _dynamicMarkerProviders.Values)
		{
			...
			{
				DynamicMapsPlugin.Log.LogError(
					$"Dynamic marker provider {dynamicProvider} threw exception in ChangeMap");
				DynamicMapsPlugin.Log.LogError($"  Exception given was: {e.Message}");
				DynamicMapsPlugin.Log.LogError($"  {e.StackTrace}");
			}
		}
